	...
	implements IFormController, EventListener<Event>, WTableModelListener, IProcessUI
{
	...
	// PM - 03-2015
	// -->
	private Label labelNextPayDate = new Label();
	private WDateEditor fieldNextPayDate = new WDateEditor();	
	// <--
	 ...
	 *  @throws Exception
	 *  
	 *  Modify by P.Massing(Hans Auler GmbH) 2015-03 - insert NextPayDate
	...
	private void zkInit() throws Exception
	{
		...
		row = rows.newRow();
		row.appendChild(labelNextPayDate.rightAlign());
		row.appendChild(fieldNextPayDate.getComponent());
		row.appendChild(new Space());
	 ...
	 *  - Init Table
	 *  
	 *  Modify by P.Massing(Hans Auler GmbH) 2015-03 - fieldNextPayDate
	 *  
	 */
	private void dynInit()
	{
		...
		fieldNextPayDate.setMandatory(true);
		fieldNextPayDate.setValue(new Timestamp(System.currentTimeMillis()));
	}   //  dynInit
	 ...
	 *  
	 *  Modify by P.Massing(Hans Auler GmbH) 2015-03 - modify Method loadTableInfo
	 *  new filter nextPayDate order by bpartner new column bpartner-value
	 ...
	 *  @param e event
	 *  Modify by P.Massing(Hans Auler GmbH) 2015-03 - insert new e.getColumn() == 7 
	 *  
	 */
	...
	/**
	 *  Calculate payamt in row.
	 *  
	 *  Created by P.Massing(Hans Auler GmbH) 2015-03
	 *  by modify the discount, new payamt is calculated 
	 */
	public void calculatepayamt()
	{
		calculatePayamt(miniTable);
	}
	 ...
	 *  Generate PaySelection
	 *  Created by P.Massing(Hans Auler GmbH) 2015-03
	 *  Insert new function to check for negative payments in current selection
	 */
...
//		miniTable.setSelectedIndices(new int[]{0});
		...
		// -->
		String msgchkn = chkNegativePaySelection(miniTable);
		if(msgchkn.length()>0){
			FDialog.error(m_WindowNo, form, "PaySelectionError", msgchkn);
			return;
		}
		// <--
