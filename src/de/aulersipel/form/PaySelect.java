...
/*
 * Modify by P.Massing(Hans Auler GmbH) 2015-03
 * new Column bpvalue
 * discount is editable
 * check for negative payments
 * only posted documents are displayed (posted='Y')
 * 
 */

package de.aulersipel.form;
...
public class PaySelect
{
	...
	public void prepareTable(IMiniTable miniTable)
	{
		...
		m_sql = miniTable.prepareTable(new ColumnInfo[] {
			...
			new ColumnInfo(Msg.translate(ctx, "C_BPartner_ID"), "bp.value", String.class, true, false, null),
			...
			// PM - 03-2015 - set DiscountAmt editable 
			new ColumnInfo(Msg.translate(ctx, "DiscountAmt"), "paymentTermDiscount(i.GrandTotal,i.C_Currency_ID,i.C_PaymentTerm_ID,i.DateInvoiced, ?) as discountamt", BigDecimal.class, false, false, null),
			// PM - 03-2015 - get discountdate
			...
			new ColumnInfo(Msg.getMsg(ctx, "DiscountDate"), "as1_paymenttermdiscountdate(i.C_PaymentTerm_ID,i.DateInvoiced,i.DateInvoiced,'N','Y')", Timestamp.class),
			...
			//	WHERE
			// PM - 03-2015 - AND i.posted='Y'
			//"i.IsSOTrx=? AND IsPaid='N'"
			"i.IsSOTrx=? AND IsPaid='N'AND i.posted='Y'"
			...
			KeyNamePair bpartner, KeyNamePair docType, IMiniTable miniTable, Timestamp nextPayDate,KeyNamePair org)
	{
		...
		if ((nextPayDate != null) && (nextPayDate.after(payDate)))
		{
			sql += "  AND (as1_paymenttermdiscountdate(i.C_PaymentTerm_ID,i.DateInvoiced,i.DateInvoiced,'N','Y') < ? ";
			sql += " OR ( as1_paymenttermdiscountdate(i.C_PaymentTerm_ID,i.DateInvoiced,i.DateInvoiced,'N','Y') = i.DateInvoiced" +
					" AND paymentTermDueDate(i.C_PaymentTerm_ID, i.DateInvoiced) < ?)) " ;
		}
		...
		if (c_doctype_id   != 0)
			sql += " AND i.c_doctype_id =?";
		sql += " ORDER BY i.C_BPartner_ID,2,3";
		if (log.isLoggable(Level.FINEST)) log.finest(sql + " - C_Currency_ID=" + bi.C_Currency_ID + ", C_BPartner_ID=" + C_BPartner_ID + ", C_doctype_id=" + c_doctype_id  );
		//  Get Open Invoices
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try
		{
			...
			if ((nextPayDate != null) && (nextPayDate.after(payDate)))
			{
				pstmt.setTimestamp(index++, nextPayDate);		
				pstmt.setTimestamp(index, nextPayDate);		
			}
	...
	/*
	 * P.Massing(Hans Auler GmbH) 2015-03
	 * calculate Payamt after change discount
	 */
	public void calculatePayamt(IMiniTable miniTable)
	{
		...
		for (int i = 0; i < rows; i++)
		{
			...
			BigDecimal payAmt = amt.subtract(discountamt);
			miniTable.setValueAt(payAmt, i, 10);
		}
	...
	*/
	...
	public LinkedList getSelectedRows(IMiniTable miniTable)
	{

		int rows = miniTable.getRowCount();
		LinkedList lli	= new LinkedList<Integer>();
	 ...
	 * Check for negative payments
	 */
	public String chkNegativePaySelection(IMiniTable miniTable)
	{

		Hashtable<String, BigDecimal> numbers
	     = new Hashtable<String, BigDecimal>();
		
		@SuppressWarnings("rawtypes")
		LinkedList lli	= new LinkedList<Integer>();
		lli = getSelectedRows(miniTable);
		
		// calculate payments
		for (int i=0; i< lli.size();i++)
		{

			IDColumn id = (IDColumn)miniTable.getValueAt((Integer)lli.get(i), 0);

			if(id.isSelected()){
			
				String bp_Value = (String)miniTable.getValueAt((Integer)lli.get(i), 2);

				if (numbers.containsKey(bp_Value)){
					BigDecimal value = new BigDecimal(numbers.get(bp_Value).toString());		
					numbers.put(bp_Value, value.add((BigDecimal)miniTable.getValueAt((Integer)lli.get(i), 10)));
				}
				else{
					numbers.put(bp_Value, (BigDecimal)miniTable.getValueAt((Integer)lli.get(i), 10));
				}
		...
		for ( String payment : numbers.keySet() ){
			if(numbers.get(payment).floatValue() <= 0.0 ){
				temp.append(payment);
				temp.append(": ");
				temp.append(numbers.get(payment));
				temp.append("\n");
			}
		}
	...
	//	Trx trx = Trx.get(trxName, true);	trx needs to be committed too
		...
		for (int i = 0; i < rows; i++)
		{
			...
			if (id.isSelected())
			{
				...
				//
				psl.xsetInvoice(C_Invoice_ID, isSOTrx,
